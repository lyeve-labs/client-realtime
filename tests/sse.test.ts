...
describe('SSEClient', () => {
  ...
  describe('constructor', () => {
    ...
    it('uses default endpoint when not specified', () => {
      ...
      expect(latestEs().url).toBe('http://localhost:3001/api/v1/realtime/events');
    });
    ...
    it('builds URL with event_types filter param', () => {
      ...
      expect(url.searchParams.get('event_types')).toBe('after_create,after_update');
    });
  ...
  describe('disconnect()', () => {
    ...
    it('clears latestEvent and lastError does not persist from prior connect', () => {
      ...
      latestEs().simulateMessage(
        JSON.stringify({ event_type: 'after_create', schema: 'x', data: {} } satisfies HookBusEvent),
      );
