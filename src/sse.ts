...
export type HookEventType =
	...
	| 'before_delete' | 'after_delete' | 'before_request' | 'after_response';
...
export class SSEClient {
	...
	constructor(config: { baseUrl: string; endpoint?: string; options?: SSEOptions }) {
	...
	connect(): void {
		if (this.#eventSource && (this.status === 'connected' || this.status === 'connecting')) return;
		this.#mounted = true;
		...
		if (event_types && event_types.length > 0) url.searchParams.set('event_types', event_types.join(','));
		if (schemas && schemas.length > 0) url.searchParams.set('schemas', schemas.join(','));
	...
	#setStatus(s: SSEStatus): void { this.status = s; this.#onStatusChange?.(s); }
	...
	#scheduleReconnect(): void {
		...
		this.#reconnectTimer = setTimeout(() => { if (this.#mounted) this.connect(); }, delay);
	}
